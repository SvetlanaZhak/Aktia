...
export async function handleGetAgreements(_r: Request, _rt: ResponseToolkit): Promise<Agreement[]> {
  const agreements = await AgreementModel.findAll();
  return agreements;
}

export async function handleGetAgreement(request: Request, _rt: ResponseToolkit): Promise<Agreement> {
